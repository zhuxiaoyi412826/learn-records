...
刚开始学习时，我整天看视频，做大量笔记，努力记忆。我还花时间实现数据结构及其相关算法。比如我会花10个小时看哈希表的视频。

随着时间推移，我意识到自己开始忘记一些事情。我没有复习笔记，感觉所有花在看视频上的时间大多白费了。
...
- **转到下一个话题**
- 有时间时复习抽认卡
- 几天后，再看一个关于这个话题的视频，大约30分钟左右
- 再过一周，再看一个关于这个话题的视频，可能是同一个视频或更短的
- 继续复习抽认卡

这里需要注意的是重复。花一两天时间看某个主题的视频，只会强化你最近几个小时学到的东西。这并不是强有力的知识强化。你会忘记的。

我会回头查看所有笔记，并把它们放进抽认卡里。很多（太多）的抽认卡。

你必须在第一次学习和之后复习之间留出一个时间间隔。

关键是：在**多个时间间段内复习一个主题**。它会不断强化和提醒你，你不会忘记。时间间隔可能会随着时间推移而变长。这被称为“[间隔重复](https://en.wikipedia.org/wiki/Spaced_repetition)”（感谢评论者）。
