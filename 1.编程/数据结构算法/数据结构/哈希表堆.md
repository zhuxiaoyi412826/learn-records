...
![](C:\Users\DELL\Desktop\笔记\数据结构算法\img\数据结构\哈希算法.png)
...
![](C:\Users\DELL\Desktop\笔记\数据结构算法\img\数据结构\MurmurHash.png)

| 特性     | MurmurHash         | MD5/SHA-256          |
| -------- | ------------------ | -------------------- |
| 类型     | 非加密哈希         | 加密哈希             |
| 速度     | 极快（GB/s）       | 慢（MB/s）           |
| 碰撞概率 | 极低（设计优化）   | 极低（密码学安全）   |
| 可逆性   | 可反向构造         | 不可反向             |
| 抗攻击   | 弱（可构造碰撞）   | 强（密码学安全）     |
| 用途     | 哈希表、分片、去重 | 密码、签名、数据校验 |
