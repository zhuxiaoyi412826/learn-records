...
方法封装了由一系列语句所描述的运算符 方法需要参数 并根据参数计算出某种数据类型的返回值

方法只能返回一个值，但可以包含多个返回语句

需要传回多数据时，可用数组、对象封装多个数据统一返回
...
| 对比项   | 静态方法 (static 修饰)         | 实例普通方法 (无 static)            |
| -------- | ------------------------------ | ----------------------------------- |
| 所属主体 | 归属类                         | 归属实例对象                        |
| 调用方式 | 类名。方法名 ()，可不创建对象  | 必须 new 对象，对象。方法名 () 调用 |
| 访问成员 | 仅能直接访问静态变量、静态方法 | 可访问静态、实例变量与实例方法      |
| 关键字   | 方法头加 static                | 不加 static 修饰                    |
| 多态特性 | 无重写多态                     | 支持重写、多态机制                  |
| 加载时机 | 类加载时随类初始化             | 创建对象时才分配方法运行环境        |
