...
Set 对于值相同的对象只会保存一个 而Map是一个关联数组  对象之间关联   

自动调整大小  

list 必须按照元素插入顺序报错 set中不能存在重复元素  queue则按照排队规则来输出元素 

arraylist  一个数值查找某个对象  map 一个对象查找另一个对象
...
这行代码用于输出`collection`的内容。`System.out.println`是Java中用于打印信息到控制台的方法。当打印一个集合时，Java会自动调用集合的`toString`方法来生成一个字符串表示，展示集合中的所有元素
...
作用：**遍历集合 collection 中每一个 Integer 元素，依次打印**
...
public class AddingGroups {
  public static void main(String[] args) {
    ...
    collection.addAll(Arrays.asList(moreInts)); //  将moreInts数组中的所有元素添加到collection集合中     Arrays.asList(moreInts)将数组转换为List集合     addAll()方法将List中的所有元素添加到目标集合中
      System.out.println(collection);
    // Runs significantly faster, but you can't
    // construct a Collection this way:
    Collections.addAll(collection, 11, 12, 13, 14, 15); //  使用Collections.addAll()方法向集合中添加多个元素     添加11, 12, 13, 14, 15这5个整数到集合中
      System.out.println(collection);
    Collections.addAll(collection, moreInts); //  再次使用Collections.addAll()方法，将moreInts数组中的所有元素添加到集合中
      System.out.println(collection);
    // Produces a list "backed by" an array:
    List<Integer> list = Arrays.asList(16,17,18,19,20);
    list.set(1, 2);  //  使用set方法替换列表中指定位置的元素 将索引为1的位置的元素值设置为2
      System.out.println(list);
    // Produces a list that is modifiable:
...
java.util 中的Arrays 和Collection  包含一些工具方法向一个Coolection中添加一组元素    Arrays.**asList()**方法可以说接受一个数组，或者一个用逗号分割的元素列表(可变参数)，并将其转换为一个list对象  

Coolection.**addAll()** 方法接受一个Collection对象，一个数组，或者一个用逗号分隔的列表，将其中所有的元素都添加到这个Coolection中



Arrays.asList   但list的底层实现使用的是数组大小无法改变
...
整个集合架构围绕**接口、抽象类、实现类**三层设计

1. **接口**：制定一套行为规范，统一对外调用标准（面向接口编程）
2. **抽象类**：抽取所有子类重复代码，减少实现类重复开发（模板 / 骨架模式）
3. **实现类**：落地底层数据结构，完成全部抽象方法，开发者 `new` 创建对象使用

**一、第一层：顶层接口（只定义标准，无实现）**
...
**二、第二层：抽象类（骨架实现，复用公共代码）**

`java.util.AbstractCollection`、`AbstractList`、`AbstractSet`、`AbstractQueue` 作用：实现上层接口，把所有集合通用逻辑写好，子类只需要实现少量抽象方法即可。

1. **AbstractCollection** 实现 Collection 接口，提供 `contains()`、`remove()`、`isEmpty()`、`toArray()` 通用逻辑
2. **AbstractList** 继承 AbstractCollection，实现 List 接口，封装索引遍历、子列表、迭代器公共逻辑
3. **AbstractSet** 继承 AbstractCollection，实现 Set 接口，重写 equals/hashCode 处理去重逻辑
4. **AbstractQueue** 继承 AbstractCollection，实现 Queue 接口，封装入队出队公共方法

> 抽象类特点：一部分方法已经写好，还有抽象方法留给子类重写，不能直接 new。

**三、第三层：具体实现类（可直接 new 使用）**

继承对应抽象类，补全剩余抽象方法，提供底层存储结构（数组 / 链表 / 哈希表）。

**List 分支**

1. ArrayList 继承 AbstractList，底层动态数组
2. LinkedList 继承 AbstractSequentialList（AbstractList 子类），底层双向链表
3. Vector 继承 AbstractList，线程安全数组

**Set 分支**

1. HashSet 继承 AbstractSet，底层 HashMap 哈希表去重
2. TreeSet 继承 AbstractSet，底层 TreeMap 红黑树，自动排序
3. LinkedHashSet 继承 HashSet，有序哈希集合

**Queue 分支**

1. ArrayDeque：继承 AbstractCollection，双端数组队列
2. PriorityQueue：继承 AbstractQueue，优先级堆队列
...
![Collection](C:\Users\Administrator\Desktop\文件\笔记\Java\javase\java进阶\img\集合\Collection.png)
...
![迭代器迭代过程](C:\Users\Administrator\Desktop\文件\笔记\Java\javase\java进阶\img\集合\迭代器迭代过程.png)
...
![迭代器迭代原理](C:\Users\Administrator\Desktop\文件\笔记\Java\javase\java进阶\img\集合\迭代器迭代原理.png)
...
![迭代器迭代原理1](C:\Users\Administrator\Desktop\文件\笔记\Java\javase\java进阶\img\集合\迭代器迭代原理1.png)
...
![并发修改异常](C:\Users\Administrator\Desktop\文件\笔记\Java\javase\java进阶\img\集合\并发修改异常.png)
...
![增强for循环](C:\Users\Administrator\Desktop\文件\笔记\Java\javase\java进阶\img\集合\增强for循环.png)
...
![1706520686621](D:\资料\1.javase\尚硅谷2024新版Java基础\下\笔记\03_笔记\模块19_集合\img\1706520686621.png)
...
![泛型](C:\Users\Administrator\Desktop\文件\笔记\Java\javase\java进阶\img\集合\泛型.png)
